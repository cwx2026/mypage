 ...
 *
 * 说明：构建出的页面里所有链接都是带站点域的绝对路径，但采用协议相对写法
 * （//域名/...），这样页面通过 HTTP 或 HTTPS 打开时，CSS/JS/图片都会跟随页面
 * 所用协议加载——GitHub Pages 的自定义域名证书尚未签发时，HTTP 下也能正常渲染。
 */

/* ================= 配置 ================= */

/** 站点根地址（末尾带 /）。部署在 GitHub Pages 子路径时必填。 */
define('SITE_URL', 'https://www.20051023.xyz/');

/** 自定义域名（留空 = 不绑定，用上面的默认地址；绑定后会在 docs/CNAME 写入该域名）。 */
define('CUSTOM_DOMAIN', 'www.20051023.xyz');
...
/** 生成带站点前缀的链接（协议相对：跟随页面所用协议，见顶部说明） */
function site_url($path = '') {
    $base = preg_replace('#^[a-z][a-z0-9+.-]*://#i', '//', SITE_URL);
