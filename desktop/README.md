...
2. **托盘图标右键菜单**：
   ...
   - 卸载博客管家… → 弹确认框，确认后停服务、删桌面/开机快捷方式并延时删除整个安装目录
...
1. **选择安装目录**：安装第一步会显示默认目录 `%LOCALAPPDATA%\博客管家`，可直接改路径或用「浏览…」选位置；取消则退出安装。
2. **自动拉取内容**：装到所选目录后，首次自动 `git clone` 拉取最新源码 + 内容 + 静态站；已有站点则 `git pull` 更新。
3. **GitHub 登录（只需一次）**：填一次用户名 + 个人访问令牌（PAT，在 github.com/settings/tokens 生成，勾选 `repo` 权限），存入本机 `%USERPROFILE%\.git-credentials`，之后发布全自动。
   - **同一台电脑重装会自动跳过**：重装时会先检查本机已存凭据，用 GitHub API 验证有效就直接跳过登录，不再反复询问。想换账号/换令牌时，删除 `%USERPROFILE%\.git-credentials` 再重装即可。
4. 装完自动启动托盘「博客管家」，桌面出现快捷方式。
...
**卸载**：托盘菜单「卸载博客管家…」→ 确认后停止本站服务、删除桌面/开机快捷方式、延时删除整个安装目录（`%LOCALAPPDATA%\博客管家` 或安装时选的目录）。GitHub 上的内容不受影响；仅本机未发布的改动会丢失。GitHub 令牌保留在 `%USERPROFILE%\.git-credentials`，如需移除请手动删除该文件。
